...
def test_equity_curve_is_interface_only() -> None:
    ...
    assert get_type_hints(getattr(EquityCurve.timestamps, "fget"))["return"] == tuple[
        datetime, ...
    ]
    assert get_type_hints(getattr(EquityCurve.equity, "fget"))["return"] == tuple[
        Decimal, ...
    ]
    assert get_type_hints(getattr(EquityCurve.drawdown, "fget"))["return"] == tuple[
        Decimal, ...
    ]
